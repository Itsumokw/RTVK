...
// 常量定义
const int SAMPLE_WIDTH = 1280;
const int SAMPLE_HEIGHT = 720;
...
private:
    ...
    void createTestTriangleGeometry();
...
void VulkanRenderer::init() {
    ...
    loadModel("C:/Users/赵亮哲/source/repos/rtvk/rtvk/models/asschercut-mesh.obj");
    loadModel("C:/Users/赵亮哲/source/repos/rtvk/rtvk/models/venus-mesh.obj");

    loadModel("C:/Users/赵亮哲/source/repos/rtvk/rtvk/models/bunny-mesh.obj");
    loadModel("C:/Users/赵亮哲/source/repos/rtvk/rtvk/models/dragon-mesh.obj");
    createEnvironmentMap("D:/vulkan/CG-FinalProject/envmap.jpg");
    setupSceneLights();
    //createTestTriangleGeometry();
    createCameraBuffer();
...
void VulkanRenderer::loadShaders() {
    try {
        // 加载光线生成着色器
        auto rgenCode = ResourceManager::readSPIRV("D:/vulkan/repos/samplevk/samplevk/shaders/raygen.spv");
        ...
        // 加载未命中着色器
        auto rmissCode = ResourceManager::readSPIRV("D:/vulkan/repos/samplevk/samplevk/shaders/rmiss.spv");
        ...
        // 加载最近命中着色器
        auto rchitCode = ResourceManager::readSPIRV("D:/vulkan/repos/samplevk/samplevk/shaders/closethit.spv");
...
void VulkanRenderer::createRayTracingPipeline() {
    ...
    hitGroup.closestHitShader = 2; // shaderStages索引
    ...
    pipelineInfo.maxPipelineRayRecursionDepth = 3; // 最大递归深度
...
void VulkanRenderer::createSBT() {
    ...
    m_sbt.missRegion.size = alignUp(handleSize, baseAlignment);
    m_sbt.hitRegion.size = alignUp(handleSize, baseAlignment);

    // 步长设置为相同大小（每组一个着色器）
    m_sbt.rgenRegion.stride = m_sbt.rgenRegion.size;
    m_sbt.missRegion.stride = m_sbt.missRegion.size;
    ...
    if (vkGetRayTracingShaderGroupHandlesKHR(
        m_device, m_rayTracingPipeline, 0, 3, handles.size(), handles.data()) != VK_SUCCESS) {
    ...
    memcpy(pData + m_sbt.rgenRegion.size + m_sbt.missRegion.size, handles.data() + 2 * handleSize, handleSize); // Hit
...
void VulkanRenderer::createAccelerationStructures() {
    ...
    glm::vec3 corners[4] = {
        glm::vec3(1.2f, 0.0f, -1.f),  // 左下
        glm::vec3(-0.f, 0.5f, -1.f),   // 右下
        glm::vec3(2.5f, 0.5f, .0f),   // 左上
        glm::vec3(-3.5f, 0.0f, -1.f)     // 右上
    };
    ...
    glm::vec3 rotations[4] = {
        glm::vec3(20.0f, 0.0f, 0.0f),    // 无旋转
        glm::vec3(0.0f, 45.0f, 0.0f),   // 绕Y轴旋转45度
        glm::vec3(30.0f, 50.0f, 0.0f),  // 绕X轴旋转30度
        glm::vec3(0.0f, 90.0f, 00.0f)   // 复合旋转
    };
    ...
    for (auto& geom : m_scene.geometries) {
        ...
        instance.flags = VK_GEOMETRY_INSTANCE_TRIANGLE_FACING_CULL_DISABLE_BIT_KHR
            | VK_GEOMETRY_INSTANCE_FORCE_OPAQUE_BIT_KHR; // 简化        
...
void VulkanRenderer::loadModel(const std::string& path) {
    ...
    for (const auto& shape : shapes) {
        ...
        for (const auto& index : shape.mesh.indices) {
            ...
            geom.indices.push_back(static_cast<uint32_t>(geom.indices.size()));
        }

        for (size_t i = 0; i < geom.indices.size(); i += 3) {
            auto& v0 = geom.vertices[geom.indices[i]];
            auto& v1 = geom.vertices[geom.indices[i + 1]];
            auto& v2 = geom.vertices[geom.indices[i + 2]];
...
void VulkanRenderer::createCameraBuffer() {
    // 根据模型尺寸计算相机位置
    glm::vec3 modelSize = m_modelMax - m_modelMin;
    ...
    // 计算视图矩阵（从模型中心看向模型）
    glm::vec3 cameraPos = glm::vec3(0.f, 0.5f, 4.0f);
    glm::mat4 view = glm::lookAt(
        cameraPos,              // 相机位置
        //glm::vec3(5.f,-1.f,-9.f),           // 观察目标 (模型中心)
...
void VulkanRenderer::createTestTriangleGeometry() {
    Geometry geom;

    // 定义三角形顶点
    // 创建一个大的三角形（覆盖大部分屏幕）
    std::vector<SimpleVertex> vertices = {
        {{ 0.0f,  2.0f, 0.f}},  // 顶部顶点向前
        {{-1.f, -1.f, 0.f}},  // 左下向前
        {{ 1.f, -1.f, 0.f}}   // 右下向前
    };
    //测试三角形时记得取消注释
    ...
    // 更新包围盒范围（用于相机位置计算）
    m_modelMin = glm::vec3(-1.0f, -1.0f, -0.1f);
    m_modelMax = glm::vec3(1.0f, 1.0f, 0.1f);
    m_modelCenter = (m_modelMin + m_modelMax) * 0.5f;
...
void VulkanRenderer::setupSceneLights() {
    ...
    sunLight.position = glm::vec4(-0.5f, -3.0f, 0.5f, 0.0f); // w=0 表示方向
    sunLight.color = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f); // 白色, 强度1.0
    ...
    //pointLight.position = glm::vec4(2.0f, -0.5f, 10.f, 1.0f); // w=1 表示位置
    //pointLight.color = glm::vec4(1.0f, 0.7f, 0.3f, 0.01f); // 橙黄色, 强度0.8
